...
func main() {
	...
	kratosAdapter := &kratos.Adapter{
		PublicURL: cfg.Kratos.PublicURL,
		HTTP:      httpClient,
	}

	issuer, err := assertion.New(
		cfg.Actor.PrivateKeyFile,
		cfg.Actor.Issuer,
		time.Duration(cfg.Actor.TTLSeconds)*time.Second,
	)
	if err != nil {
		log.Fatal(err)
	}

	credentials, err := grpcinterface.Credentials(
	...
	server := grpc.NewServer(
		...
		grpc.UnaryInterceptor(grpcinterface.AuthorizeCaller(cfg.GRPC.TrustedCaller)),
	)
