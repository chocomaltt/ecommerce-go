...
func (v *Verifier) Verify(token string) (port.Actor, error) {
	...
	if !ed25519.Verify(v.publicKey, []byte(signedPayload), signature) {
	...
	if claims.Subject == "" || claims.Issuer != v.issuer || claims.Audience != v.audience || time.Now().Unix() >= claims.ExpiresAt {
