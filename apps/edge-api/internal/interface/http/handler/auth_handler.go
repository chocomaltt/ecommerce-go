...
func (h *AuthHandler) Register(c *gin.Context) {
	request, err := util.GetBody[RegisterRequest](c, "body")
	if err != nil {
		response.BadRequest(c, "invalid request", err)
		return
	}

	authentication, err := h.identity.Register(c.Request.Context(), request.Email, request.Password)
	if err != nil {
	...
	response.Created(c, "user registered successfully", authResponse{
		User:         toUser(authentication.User),
		SessionToken: authentication.SessionToken,
	})
}

func (h *AuthHandler) Login(c *gin.Context) {
	request, err := util.GetBody[LoginRequest](c, "body")
	if err != nil {
		response.BadRequest(c, "invalid request", err)
		return
	}

	authentication, err := h.identity.Login(c.Request.Context(), request.Email, request.Password)
	if err != nil {
...
func (h *AuthHandler) OrderCaller(c *gin.Context) {
	...
	response.OK(c, "authenticated gRPC caller", gin.H{
		"user_id":        caller.UserID,
		"email":          caller.Email,
		"caller_service": caller.Service,
	})
}

func toUser(user port.User) userResponse {
	return userResponse{
		ID:    user.ID,
		Email: user.Email,
	}
